...
struct Level {
  ...
  inline Level(std::string const &name, uint32_t id = 0) : name(name), id(id) {}
};
...
public:
  ...
  inline virtual json::object_t saveExtension() const { return {}; }
...
public:
  ...
  static Self importData(std::vector<uint8_t> const &data,
    ...
    if (!replayJson["description"].is_null())
      replay.description = replayJson["description"];
    if (!replayJson["version"].is_null())
      replay.version = replayJson["version"];
    if (!replayJson["duration"].is_null())
      replay.duration = replayJson["duration"];
    if (!replayJson["author"].is_null())
      replay.author = replayJson["author"];
    if (!replayJson["seed"].is_null())
      replay.seed = replayJson["seed"];
    if (!replayJson["coins"].is_null())
      replay.coins = replayJson["coins"];
    if (!replayJson["ldm"].is_null())
      replay.ldm = replayJson["ldm"];

    if (replayJson.contains("lastRecordedFrame") &&
        !replayJson["lastRecordedFrame"].is_null())
      replay.lastRecordedFrame = replayJson["lastRecordedFrame"];

    if (!replayJson["bot"]["name"].is_null())
      replay.botInfo.name = replayJson["bot"]["name"];
    if (!replayJson["bot"]["version"].is_null())
      replay.botInfo.version = replayJson["bot"]["version"];
    if (!replayJson["level"]["id"].is_null())
      replay.levelInfo.id = replayJson["level"]["id"];
    if (!replayJson["level"]["name"].is_null())
    ...
    bool rotation = ver.find("beta.") == std::string::npos &&
    ...
    for (json const &frameFixJson : replayJson["frameFixes"]) {
      ...
      if (frameFixJson.contains("player1")) {
        ...
        frameFix.p1.pos =
            ccp(frameFixJson["player1X"], frameFixJson["player1Y"]);
        frameFix.p1.rotate = false;

        frameFix.p2.pos =
            ccp(frameFixJson["player2X"], frameFixJson["player2Y"]);
        frameFix.p2.rotate = false;
  ...
  std::vector<uint8_t> exportData(bool exportJson = false) {
    ...
    if (lastRecordedFrame > 0)
      replayJson["lastRecordedFrame"] = lastRecordedFrame;
