...
namespace {
void setInputLockedState(CCTextInputNode *input, bool locked,
                         std::string const &enabledID) {
  if (!input)
    return;

  input->setID(locked ? "disabled-input"_spr : enabledID);
  input->setMouseEnabled(!locked);
  input->setTouchEnabled(!locked);
  ...
  auto color = locked ? ccc3(165, 165, 165) : ccc3(255, 255, 255);
  auto opacity = locked ? static_cast<GLubyte>(150) : static_cast<GLubyte>(255);
  auto placeholderColor = locked ? ccc3(110, 110, 110) : ccc3(163, 135, 121);

  input->setLabelNormalColor(color);
  input->setLabelPlaceholderColor(placeholderColor);

  input->updateLabel(input->getString());

  input->m_textField->setColor(color);
  input->m_textField->setOpacity(opacity);

  if (input->m_textLabel) {
    input->m_textLabel->setColor(color);
    input->m_textLabel->setOpacity(opacity);
  }

  if (input->m_cursor) {
    if (locked) {
      input->m_cursor->setVisible(false);
      input->m_cursor->setOpacity(0);
    } else {
      input->m_cursor->setVisible(true);
      input->m_cursor->setOpacity(255);
    }
  }
}
} // namespace
...
bool RenderSettingsLayer::setup() {
  ...
  cocos2d::CCSize center =
      cocos2d::CCDirector::sharedDirector()->getWinSize() / 2;
  ...
  lbl = CCLabelBMFont::create("HDR:", "bigFont.fnt");
  lbl->setPosition(ccp(155, -32));
  lbl->setAnchorPoint({0, 0.5});
  lbl->setOpacity(200);
  lbl->setScale(0.325);
  menu->addChild(lbl);
  ...
  toggle = CCMenuItemToggler::create(
      ...
      CCSprite::createWithSpriteFrameName("GJ_checkOn_001.png"), this,
      menu_selector(RecordLayer::toggleSetting));
  toggle->setPosition(ccp(205, -32));
  toggle->setScale(0.555);
  toggle->toggle(mod->getSavedValue<bool>("render_hdr"));
  toggle->setID("render_hdr");
  menu->addChild(toggle);

  auto hdrCodecSpr = ButtonSprite::create(
      mod->getSavedValue<std::string>("render_hdr_codec", "NVENC").c_str());
  hdrCodecSpr->setScale(0.5f);
  hdrCodecBtn = CCMenuItemSpriteExtra::create(
      hdrCodecSpr, this, menu_selector(RenderSettingsLayer::onHDRCodec));
  hdrCodecBtn->setPosition(ccp(205, -58));
  menu->addChild(hdrCodecBtn);
  ...
  spr2->setScale(0.33f);
  btn = CCMenuItemSpriteExtra::create(
      spr2, this, menu_selector(RenderSettingsLayer::showInfoPopup));
...
void RenderSettingsLayer::showInfoPopup(CCObject *obj) {
  ...
  std::string msg =
      tag ? "Adds only the original level song to the video, ignoring all song "
            "and SFX triggers."
          : "Records the game's audio in another attempt and adds it to the "
            "video. This ensures all song and SFX triggers are captured.";
  FLAlertLayer::create(title.c_str(), msg.c_str(), "Ok")->show();
}

void RenderSettingsLayer::updateManualArgState() {
  bool renderHDR = mod->getSavedValue<bool>("render_hdr");
  bool lockExtraArgs = Renderer::shouldUseAPI() || renderHDR;
  bool lockVideoArgs = renderHDR;

  setInputLockedState(argsInput, lockExtraArgs, "render-extra-args-input"_spr);
  setInputLockedState(videoArgsInput, lockVideoArgs,
                      "render-video-args-input"_spr);

  if (hdrCodecBtn)
    hdrCodecBtn->setVisible(renderHDR);
}

void RenderSettingsLayer::onHDRCodec(CCObject *obj) {
  std::string current =
      Mod::get()->getSavedValue<std::string>("render_hdr_codec", "NVENC");
  std::string next = (current == "NVENC") ? "Libx265" : "NVENC";
  Mod::get()->setSavedValue("render_hdr_codec", next);

  auto btn = static_cast<CCMenuItemSpriteExtra *>(obj);
  if (auto spr = typeinfo_cast<ButtonSprite *>(btn->getNormalImage())) {
    spr->setString(next.c_str());
  }
}
