...
class $modify(FLAlertLayer) {
  ...
  virtual bool ccTouchBegan(cocos2d::CCTouch *touch, cocos2d::CCEvent *event) {
    ...
    for (size_t i = 0; i < layer->spriteButtons.size(); i++) {
      ...
      cocos2d::CCSize btnSize = layer->spriteButtons[i]->getContentSize() *
                                layer->spriteButtons[i]->getScale();
  ...
  virtual void ccTouchMoved(cocos2d::CCTouch *touch, cocos2d::CCEvent *event) {
    ...
    cocos2d::CCPoint position =
        touch->getLocation() - layer->movingButton.offset;
...
bool ButtonEditLayer::setup() {
  ...
  CCMenuItemSpriteExtra *btn = CCMenuItemSpriteExtra::create(
      spr, this, menu_selector(ButtonEditLayer::onSave));
  ...
  btn = CCMenuItemSpriteExtra::create(spr, this,
                                      menu_selector(ButtonEditLayer::onClose));
  ...
  btn = CCMenuItemSpriteExtra::create(
      spr, this, menu_selector(ButtonEditLayer::onRestore));
  ...
  for (size_t i = 0; i < spriteButtons.size(); i++) {
    cocos2d::CCPoint position =
        ccp(mod->getSavedValue<float>(indexToID[i] + "_pos_x"),
...
void ButtonEditLayer::addSliders() {
  opacitySlider =
      Slider::create(this, menu_selector(ButtonEditLayer::updateOpacity), 0.8f);
  ...
  scaleSlider =
      Slider::create(this, menu_selector(ButtonEditLayer::updateScale), 0.8f);
...
void ButtonEditLayer::addSprites() {
  ...
  cocos2d::CCPoint pos = ccp(mod->getSavedValue<float>("button_off_pos_x"),
                             mod->getSavedValue<float>("button_off_pos_y"));

  spr->setPosition(pos);
  spr->setScale(mod->getSavedValue<float>("button_off_scale"));
  spr->setOpacity(
      static_cast<int>(mod->getSavedValue<float>("button_off_opacity") * 255));
  ...
  pos = ccp(mod->getSavedValue<float>("button_advance_frame_pos_x"),
            mod->getSavedValue<float>("button_advance_frame_pos_y"));

  spr->setPosition(pos);
  spr->setScale(mod->getSavedValue<float>("button_advance_frame_scale"));
  spr->setOpacity(static_cast<int>(
      mod->getSavedValue<float>("button_advance_frame_opacity") * 255));
  ...
  pos = ccp(mod->getSavedValue<float>("button_speedhack_pos_x"),
            mod->getSavedValue<float>("button_speedhack_pos_y"));

  spr->setPosition(pos);
  spr->setScale(mod->getSavedValue<float>("button_speedhack_scale"));
  spr->setOpacity(static_cast<int>(
      mod->getSavedValue<float>("button_speedhack_opacity") * 255));
