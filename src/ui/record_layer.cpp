...
$execute {
      ...
      "frame_offset", +[](int64_t value) {
        ...
        if (g.layer) {
          static_cast<RecordLayer *>(g.layer)->warningLabel->setString(
              ("WARNING: Currently recording / playing macros with a frame "
               "offset of " +
               std::to_string(value))
                  .c_str());
          static_cast<RecordLayer *>(g.layer)->warningLabel->setVisible(value !=
                                                                        0);
  ...
  geode::listenForSettingChanges<cocos2d::ccColor3B>(
      "background_color", +[](cocos2d::ccColor3B value) {
        ...
        if (g.layer) {
          CCArray *children =
              CCDirector::sharedDirector()->getRunningScene()->getChildren();
...
void RecordLayer::toggleContinue(CCObject *) {
  auto &g = Global::get();

  if (g.macro.inputs.empty() || g.macro.lastRecordedFrame <= 0)
    return;
...
void RecordLayer::textChanged(CCTextInputNode *node) {
  ...
  if (tpsInput && node == tpsInput) {
    float value =
        geode::utils::numFromString<float>(tpsInput->getString()).unwrapOr(0.f);
  ...
  if (audioSpeedhackInput && node == audioSpeedhackInput) {
    if (std::string_view(audioSpeedhackInput->getString()) != "") {
      std::string value = audioSpeedhackInput->getString();

      if (value == ".")
        audioSpeedhackInput->setString("0.");
      else if (std::count(value.begin(), value.end(), '.') == 2 ||
               std::stof(value) > 10)
        audioSpeedhackInput->setString(
            mod->getSavedValue<std::string>("macro_audio_speedhack").c_str());
      else
        mod->setSavedValue("macro_audio_speedhack",
                           std::string(audioSpeedhackInput->getString()));
    }
    checkAudioSpeedhack();
  }
}
...
void RecordLayer::toggleSetting(CCObject *obj) {
  ...
  if (id == "swift_click_enabled")
    g.swiftClickEnabled = value;
  if (id == "auto_swift_click_enabled")
    g.autoSwiftClickEnabled = value;
  ...
  if (id == "macro_hide_recording_label" || id == "macro_hide_playing_label" ||
      id == "render_hide_labels")
    Interface::updateLabels();
...
void RecordLayer::openKeybinds(CCObject *) {
  ...
  FLAlertLayer *layer =
      typeinfo_cast<FLAlertLayer *>(scene->getChildren()->lastObject());
  if (!layer)
...
void RecordLayer::onAutosaves(CCObject *) {
  ...
  if (std::filesystem::exists(path))
    ...
    FLAlertLayer::create("Error",
                         "There was an error getting the folder. ID: 5", "Ok")
        ->show();
  }
}
...
bool RecordLayer::setup() {
  ...
  actionsLabel = CCLabelBMFont::create(
      ("Actions: " + std::to_string(g.macro.inputs.size())).c_str(),
      "chatFont.fnt");
  ...
  btn = CCMenuItemSpriteExtra::create(btnSprite, this,
  ...
  btn = CCMenuItemSpriteExtra::create(emptyBtn, this,
...
void RecordLayer::setToggleMember(CCMenuItemToggler *toggle, std::string id) {
  ...
  if (id == "macro_audio_speedhack_enabled")
    audioSpeedhackToggle = toggle;
  if (id == "macro_show_trajectory")
...
void RecordLayer::loadSetting(RecordSetting sett, float yPos) {
  ...
  if (sett.input == InputType::AudioSpeedhack) {
    CCScale9Sprite *bg =
        CCScale9Sprite::create("square02b_001.png", {0, 0, 80, 80});
    bg->setPosition(ccp(110, yPos + 10));
    bg->setScale(0.355f);
    ...
    audioSpeedhackInput =
        CCTextInputNode::create(150, 30, "SH", "chatFont.fnt");
    audioSpeedhackInput->setPosition(ccp(127.5, yPos));
    audioSpeedhackInput->m_textField->setAnchorPoint({0.5f, 0.5f});
    audioSpeedhackInput->ignoreAnchorPointForPosition(true);
    audioSpeedhackInput->setMaxLabelScale(0.7f);
    audioSpeedhackInput->setMouseEnabled(true);
    audioSpeedhackInput->setTouchEnabled(true);
    audioSpeedhackInput->setContentSize({32, 20});
    audioSpeedhackInput->setAllowedChars("0123456789.");
    audioSpeedhackInput->setString(
        mod->getSavedValue<std::string>("macro_audio_speedhack").c_str());
    audioSpeedhackInput->setMaxLabelWidth(30.f);
    audioSpeedhackInput->setDelegate(this);
    audioSpeedhackInput->setMaxLabelLength(6);

    nodes.push_back(static_cast<CCNode *>(audioSpeedhackInput));
    menu->addChild(audioSpeedhackInput);
  }
  ...
  if (sett.input == InputType::Tps) {
    ...
    tpsInput->setString(
        Utils::getSimplifiedString(
            fmt::format("{:.3f}",
                        Mod::get()->getSavedValue<double>("macro_tps")))
            .c_str());
...
void RecordLayer::goToSettingsPage(int page) {
  ...
  size_t count = settings[page].size();
  if (count > 6)
    count = 6;
...
void RecordLayer::checkAudioSpeedhack() {
  if (audioSpeedhackInput && audioSpeedhackToggle) {
    if (std::string_view(audioSpeedhackInput->getString()) != "") {
      Mod::get()->setSavedValue("macro_audio_speedhack",
                                std::string(audioSpeedhackInput->getString()));
      if (std::stof(audioSpeedhackInput->getString()) != 1.f)
        audioSpeedhackToggle->toggle(true);
      else
        audioSpeedhackToggle->toggle(false);

      if (Global::get().state == state::none)
        Global::updatePitch(std::stof(audioSpeedhackInput->getString()));
    }
  }
}
...
void RecordLayer::updateTPS() {
  ...
  tpsInput->setString(
      Utils::getSimplifiedString(
          fmt::format("{:.3f}", Mod::get()->getSavedValue<double>("macro_tps")))
          .c_str());
