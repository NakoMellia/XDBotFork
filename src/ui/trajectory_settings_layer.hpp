...
private:
  ...
  bool setup() {
    ...
    toggle->toggle(
        Mod::get()->getSavedValue<bool>("macro_trajectory_both_sides"));
    ...
    CCMenuItemSpriteExtra *colorButton = CCMenuItemSpriteExtra::create(
        color1, this, menu_selector(TrajectorySettingsLayer::openColorPicker));
    ...
    colorButton = CCMenuItemSpriteExtra::create(
        color2, this, menu_selector(TrajectorySettingsLayer::openColorPicker));
  ...
  void onToggle(CCObject *obj) {
    ...
    Mod::get()->setSavedValue("macro_trajectory_both_sides",
                              !toggle->isToggled());
  ...
  void openColorPicker(CCObject *obj) {
    ColorChannelSprite *color =
        static_cast<CCNode *>(obj)->getTag() == 1 ? color1 : color2;
    ColorPickPopup *popup = ColorPickPopup::create(color->getColor());
    popup->setColorTarget(color);
    popup->setCallback(
        [this](const cocos2d::ccColor4B &) { this->updateColor(); });
    popup->show();
  }
